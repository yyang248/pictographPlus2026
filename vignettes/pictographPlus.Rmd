...
setwd("~/Desktop/scripts/R/pictographPlus")
library(devtools)
load_all()
# library(UpSetR)
...
```{r}
head(read.csv(system.file('extdata/examples/example1_snv.csv', package = 'pictographPlus')))
```
...
```{r}
head(read.csv(system.file('extdata/examples/example2_snv.csv', package = 'pictographPlus')))
```
...
```{r}
head(read.csv(system.file('extdata/examples/example2_snv_with_purity.csv', package = 'pictographPlus')))
```
...
```{r}
head(read.csv(system.file('extdata/examples/example3_snv.csv', package = 'pictographPlus')))
```

The CNA file should contain columns "sample", "chrom", "start", "end", "tcn" and "baf".

```{r}
head(read.csv(system.file('extdata/examples/example3_cna.csv', package = 'pictographPlus')))
```
...
```{r}
head(read.csv(system.file('extdata/examples/example4_snv.csv', package = 'pictographPlus')))
```

```{r}
head(read.csv(system.file('extdata/examples/example4_cna.csv', package = 'pictographPlus')))
```

```{r}
head(read.csv(system.file('extdata/examples/example4_SNP.csv', package = 'pictographPlus')))
```
...
```{r input_data}
# runPictograph(mutation_file=system.file('extdata/examples/example1_snv.csv', package = 'pictographPlus'),
#          copy_number_file=NULL,
#          SNV_file=NULL,
#          outputDir=system.file('extdata/examples/output', package = 'pictographPlus'))
```
## 4. Parameters
...
knitr::include_graphics(system.file('extdata/model1.png', package = 'pictographPlus'))
```

#### Model 2: use the  $icn$, $m$, and $cncf$ estimated from model 1 and estimate all parameters in all mutations

```{r, echo=FALSE, out.width="70%"}
knitr::include_graphics(system.file('extdata/model2.png', package = 'pictographPlus'))
```
